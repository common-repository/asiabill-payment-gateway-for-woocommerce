...
//        );
	...
	public function process_admin_options(){
        $test_mode = isset($_POST['woocommerce_'.$this->id.'_use_test_mode']) ? 'yes' : 'no';
	...
	public function process_payment($order_id) {
        ...
        $save_card = false; //isset($_POST['wc-'.$this->id.'-new-payment-method']);
        ...
        */
        ...
        if( empty($payment_method_id) ){
            $this->logger->error('无效的method_id');
        ...
        if( is_array($response) && isset($response['code']) ){
            if( $response['code'] == '00000' ){
                ...
                if( $save_card && $new_card && $customer->has_customer() ){
                    $this->api()->request('paymentMethods_attach',['path' => [
                        'customerPaymentMethodId' => $payment_method_id,
    ...
    public function javascript_params(){
        ...
        try {
            ...
            $token = null;
            $this->logger->error( '获取token失败：'.$e->getMessage() );
        }
