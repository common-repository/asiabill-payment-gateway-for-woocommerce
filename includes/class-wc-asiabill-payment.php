...
abstract class WC_Asiabill_Payment_Gateway extends WC_Payment_Gateway_CC {
    ...
    public function process_response($result_data,$order){
        ...
        if( $order->get_meta('_trade_no') !== $result_data['tradeNo']
            || ( $order->has_status(['on-hold','failed'])  )
        ){
            ...
            if( $get_status === 'success' ){
                ...
                $localized_message = sprintf( __( 'AsiaBill payment pending : %s, %s', 'asiabill' ), $result_data['tradeNo'], $result_data['orderInfo'] );
                $order->update_status( 'on-hold', $localized_message );
    ...
    protected function order_parameter($order){
        ...
        foreach ($order_line as $item) {
            $goods_details[] = [
                'goodsTitle' => $item->get_name(),
                'goodsCount' => $item->get_quantity(),
                'goodsPrice' => $item->get_total()/$item->get_quantity()
            ];
        }
