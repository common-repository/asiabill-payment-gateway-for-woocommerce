=== AsiaBill Payment Gateway for WooCommerce ===
Contributors: AsiaBill
Donate link: https://en.asiabill.com/
Tags: AsiaBill, credit card, bitcoin, cryptocurrencies, crypto, payment gateway, payment request, sofort, giropay, p24, paysafecard, apple pay, google pay, alipay, wechat pay, 支付宝, 微信支付, 收款, 支付, USD, CNY
Requires at least: 6.3
Tested up to: 6.4.3
Stable tag: 1.2.12
Requires PHP: 7.1
License: GPLv3
License URI: https://www.gnu.org/licenses/gpl-3.0.zh-cn.html

Take credit/debit card and other payment methods on your store using AsiaBill.

== Description ==

Accept Visa, MasterCard, American Express, JCB, Diners Club, Discover, Alipay, Wechat Pay, Cryptocurrencies, Bitcoin, Ether, USDT, DOGE, Sofort, iDeal, Giropay, P24, Paysafecard, Boleto, Klarna and more directly on your store with the AsiaBill payment gateway for WooCommerce, including Apple Pay and Google Pay for mobile and desktop.

== Installation ==

== Frequently Asked Questions ==

= Why choose AsiaBill? =
https://en.asiabill.com/

= Where can I get support or talk to your customer service team? =
https://en.asiabill.com/contact


== Screenshots ==

1. AsiaBill provides various payment methods from Credit Card Payment to Local Payment Methods.
2. The Payment Setting page is used to set up the main AsiaBill Gateway. For any queries please contact us at <a href="https://en.asiabill.com/contact">https://en.asiabill.com/contact</a>.
3. Customers can choose to pay with a saved payment method or a new card and save their card details for future purchases.
4. You can keep track of the transaction state changes as well as all the other payment information for each order in this page.

== Changelog ==
= 1.2.12 =
* Fixed a small issue

= 1.2.11 =
* Fix the order status issue in 3D

= 1.2.10 =
* Optimize the acquisition of product information in orders

= 1.2.9 =
* Pending order status capture

= 1.2.8 =
* Fixed a small issue

= 1.2.7 =
* checkout blocks support

= 1.2.6 =
* Add Bancontact Payment

= 1.2.5 =
* Add Klarna Payment

= 1.2.4 =
* Optimize some functions

= 1.2.3 =
* Fixed a small issue

= 1.2.2 =
* Fixed a small issue

= 1.2.1 =
* Adaptation woocommerce cart all in one

= 1.2.0 =
* Upgrade the new API interface

= 1.1.9 =
* Refuse to pay again from the order
* Solve the problem that test mode cannot update the status

= 1.1.8 =
* Conflict resolution
* Fixed a small issue

= 1.1.7 =
* Fixed a small issue

= 1.1.6 =
* Fixed a small issue

= 1.1.5 =
* Optimized test mode

= 1.1.4 =
* Restored known problems

= 1.1.3 =
* Inline Credit Card Form

= 1.0.2 =
* Code security optimization.

= 1.0.1 =
* Optimization of redirection.

= 1.0 =
* This is the first release.

== Upgrade Notice ==

